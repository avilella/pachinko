...
Bio::EnsEMBL::Compara::RunnableDB::Simancestral
...
my $simancestral = Bio::EnsEMBL::Compara::RunnableDB::Simancestral->new
  (
   -db         => $db,
   -input_id   => $input_id,
   -analysis   => $analysis
  );
$simancestral->fetch_input(); #reads from DB
$simancestral->run();
$simancestral->output();
$simancestral->write_output(); #writes to DB
...
package Bio::EnsEMBL::Compara::RunnableDB::Simancestral;
...
sub run {
  ...
  $self->run_expanded_genetree;
  return 1 unless (defined($self->{nhx_treefile}));
  $self->run_simancestral;
  return 1 unless (defined($self->{aln_f}) && defined($self->{cds_f}));

  $self->reference_alignment($self->{aln_f},$self->{cds_f});

  $self->{coverage} = undef;
  my $coverage = 4;

##  foreach my $q (sort {$b <=> $a} 1..10) {
  foreach my $q (sort {$a <=> $b} 0..1) {
    $self->{qscore} = $q * 4;
    $self->{qscore} = 1 if (0 == $q);
    #    $self->{coverage} = $coverage;
    print STDERR "# QSCORE " ,$self->{qscore},"\n";
    $self->simulate_from_newsamples($coverage);
    $self->align;
...
#     $self->run_jackhmmer_merge_by_merge('40000000');
#     $self->merge_fragments('40000000');
#     # $DB::single=1;1;
#     $self->indel_stats('40000000');
#     next unless defined($self->{merge_hash});
#     $self->prepare_fragments_for_assessment;
#     next unless defined($self->{pepfile});
#     $self->run_magicmirror($self->{pepfile},$self->{cds_f});
#     next if (!defined($self->{transcriptsfile}) || -z $self->{transcriptsfile});
#     $self->run_jackhmmer($self->{transcriptsfile},$self->{pepfile});
    delete $self->{read_placement_sets};
  }

  if (defined($self->{store_this})) {
    my $subdir = $self->{store_this};
    my $newsamplesfile = $self->{newsamplesfile};
    my $ret2 = `cp $newsamplesfile $subdir/`;
  }
...
sub run_expanded_genetree {
  ...
  # PRE EXPANDED TREE
  my $pre_treefile = $self->worker_temp_directory . "$node_id.prank_pre.nh";
...
#  my $newick_string = $self->{tree}->newick_format;
  ...
  open NEWICK, ">$pre_treefile" or die "$!";
  print NEWICK "$newick_string\n";
  close NEWICK;
  $self->{pre_treefile} = $pre_treefile;
  ...
  unless (defined($self->{no_N_edit})) {
    foreach my $leaf (@leaves) {
      ...
      if ($@) {
        # It may be that we have deleted all of them already
        if ($leaf->num_leaves < 2) {return 0;}
      }
      $cdna =~ s/N/\-/g;
  ...
  my $newick = $self->{tree}->newick_format('member_id_taxon_id');
  my $refnewickfile = $self->worker_temp_directory . "$node_id.nh";
  open REFTREE, ">$refnewickfile" or die "$!";
  print REFTREE $newick;
  close REFTREE;
  $self->{refnewickfile} = $refnewickfile;

  # Foreach leaf in the extended species tree
  my $large_number = 100000000;
  my $huge_number  = $self->{huge_number};
  my $flagged_global;
  my $num_new_taxa = 0;
  foreach my $taxon_subnode (@{$self->{taxon_tree}->get_all_leaves}) {
    ...
    while ($parent->has_parent) {
      ...
      foreach my $ref_subnode (@ref_subnodes) {
        ...
        my $stable_id_name = "r" . int(rand(99999999)) . "_" . $taxon_id;
        my $new_member = Bio::EnsEMBL::Compara::Member->new
          (
           -stable_id    => $stable_id_name,
           -source_name  => 'EXTERNALCDS',
           -taxon_id     => $taxon_id,
           -genome_db_id => 999999999, # FIXME
          );
        bless $new_member, "Bio::EnsEMBL::Compara::AlignedMember";
        my ($ref_child_a,$ref_child_b) = @{$ref_subnode->children};
        my $ref_dist_to_parent = $ref_subnode->distance_to_parent;
        my $dist_new_to_ref = $ref_dist_to_parent/2;
        my $dist_ref_to_subroot = $ref_dist_to_parent/2;
        my $ref_dist_a = $ref_child_a->distance_to_parent;
        my $ref_dist_b = $ref_child_b->distance_to_parent;
        my $avg_dist;
        $avg_dist = $ref_dist_a if ($ref_dist_a < $ref_dist_b);
        $avg_dist = $ref_dist_b if ($ref_dist_b <= $ref_dist_a);
        my $new_node = Bio::EnsEMBL::Compara::NestedSet->new();
        $new_node->node_id($large_number++);
        $ref_child_a->disavow_parent;
        $ref_child_b->disavow_parent;
        # attach new to ref with half old branch length
        $ref_subnode->add_child($new_node,$dist_new_to_ref);
        # divide old branch length by half
        $ref_subnode->distance_to_parent($dist_ref_to_subroot);
        # attach child_a to new
        $new_node->add_child($ref_child_a,$ref_dist_a);
        # attach child_b to new
        $new_node->add_child($ref_child_b,$ref_dist_b);

        my @leaves = @{$new_node->get_all_leaves};
        my @distances = sort map {$_->distance_to_ancestor($new_node)} @leaves;
        # take a random existing distance
        my $new_avg_dist = $distances[int(rand(scalar @distances))];
        # give the new member distance a bit of a wiggle between 0.9 - 1.1
        # random number between 0.9 and 1.1 = (1-(rand(0.1)*((-3)+2*(1+int(rand(2)%2)))))
        $new_avg_dist = $new_avg_dist * (1-(rand(0.1)*((-3)+2*(1+int(rand(2)%2)))));
        # attach the new_member to new
        $ref_subnode->add_child($new_member,$new_avg_dist);

        $num_stored++;
        $new_member->node_id($huge_number);
        $new_member->member_id($huge_number);
        $flagged_global->{$huge_number++} = 1;
      }
      last if ($num_stored >= $num_nonflagged);
    }
  }
  return 1 if ($num_new_taxa < 1);

  $self->{tree}->print_tree(0.01) if ($self->debug);
...
sub run_simancestral {
  ...
  my $eval_newick;
  eval {
    $eval_newick = Bio::EnsEMBL::Compara::Graph::NewickParser::parse_newick_into_tree($newick);
    my @leaves = @{$eval_newick->get_all_leaves};
  };
  if($@) {
    throw("Unparsable extended genetree");
  }

  print STDERR "Preparing input tree for simulation\n";
  # We do a first round of branch length enlargement due to the codon vs nt length difference
  my $total_branch_length = 0;
  foreach my $subnode ($self->{tree}->get_all_subnodes) {
    my $dist = $subnode->distance_to_parent;
    $total_branch_length += $dist;
  }
  return 1 if (0 == $total_branch_length);

  print STDERR "Cleaning input tree distances for simulation\n";
  my @subnodes = $self->{tree}->get_all_subnodes;
  foreach my $subnode (@subnodes) {
    my $dist = $subnode->distance_to_parent;
    $dist = 0.000000001 if (0 == $dist); $dist = $dist*3;
    $subnode->distance_to_parent($dist);
    $total_branch_length += $dist;
  }
  $newick = $self->{tree}->newick_format('member_id_taxon_id');
  ...
  my $aln_percent_identity = $tree->get_tagvalue('aln_percent_identity');
  ...
  my $substitution_rate;
  my $indel_rate;
  my $aln_f;
  my $cds_f;

  # Calculate initial rates
  $substitution_rate = $total_branch_length;
  $indel_rate = ($aln_length/$residues_length)/($substitution_rate);
  if ($indel_rate < 0.00000001) {
    $indel_rate = 0.00000001;
  } elsif ($indel_rate > 10) {
    $indel_rate = 10;
  }

  my $indel_rate_close_enough = 0;
  my $subst_rate_close_enough = 0;
  my $aln_length_close_enough = 0;
  my $iterations = 1;
  my $ins_rate = $indel_rate/10; # It seems like ins_rate better starts low
  my $del_rate = $indel_rate;
  my $sim_aln_num_residues;
  my $sim_aln_length;
  my $sim_aln_percent_identity;

  print STDERR "Looping input tree through simulation\n";
  while (
         0 == $indel_rate_close_enough
         || 0 == $subst_rate_close_enough
         # || 0 == $aln_length_close_enough
        ) {
    $indel_rate_close_enough = 0;
    $subst_rate_close_enough = 0;
    $aln_length_close_enough = 0;
    # print STDERR "indel_rate $indel_rate\n";
    my $low_ins_rate = sprintf("%0.30f",$ins_rate/2);
    my $low_del_rate = sprintf("%0.30f",$del_rate/2);
    my $high_ins_rate = sprintf("%0.30f",$ins_rate*2);
    my $high_del_rate = sprintf("%0.30f",$del_rate*2);

    my $output_f = $self->worker_temp_directory."$tree_id.sim";
    my $ctrl_f   = $self->worker_temp_directory."control.txt";

    $residues_length = $self->{insert_size} if ($residues_length < $self->{insert_size});
    $length = $residues_length;
    my $length1 = int($length/5);
    my $length2 = $length1;
    my $length3 = $length1;
    my $length4 = $length1;
    my $length5 = $length1;
...
[MODEL] model1
  ...
  [insertmodel] POW 1.8 100
  [deletemodel] POW 1.8 100
  [insertrate] $low_ins_rate
  [deleterate] $low_del_rate

[MODEL] model2
  [submodel]     2.5  0.5     //  Substitution model is M0 with kappa=2.5, omega=0.5
  [insertmodel] POW 1.8 100
  [deletemodel] POW 1.8 100
  [insertrate] $high_ins_rate
  [deleterate] $high_del_rate

[TREE] tree1 $newick

[PARTITIONS] partition1
    [tree1  model2  $length1]
    [tree1  model1  $length2]
    [tree1  model2  $length3]
    [tree1  model1  $length4]
    [tree1  model2  $length5]
  ...
  ^;
    ...
    print STDERR "[total_branch_length $total_branch_length del_rate $del_rate ins_rate $ins_rate residues_length $residues_length]\n" if ($self->debug);
    ...
    my $cmd = "/nfs/users/nfs_a/avilella/src/indelible/latest/INDELibleV1.02/src/indelible $ctrl_f 1>/dev/null 2>/dev/null";
    ...
    $sim_aln_num_residues = $aln->num_residues;
    $sim_aln_length = $aln->length;
    $sim_aln_percent_identity = $aln->average_percentage_identity;

    print STDERR "aln_percent_identity $sim_aln_percent_identity $aln_percent_identity\n";
    print STDERR "aln_num_residues     $sim_aln_num_residues $aln_num_residues\n";
    print STDERR "aln_length           $sim_aln_length $aln_length\n";

    if ($sim_aln_num_residues < $aln_num_residues && ((1.2 * $sim_aln_num_residues) < $aln_num_residues)) {
      $ins_rate = $ins_rate * 1.2;
      $del_rate = $del_rate * 0.8;
    } elsif ($sim_aln_num_residues > $aln_num_residues && ((0.8 * $sim_aln_num_residues) > $aln_num_residues)) {
      $ins_rate = $ins_rate * 0.8;
      $del_rate = $del_rate * 1.2;
    } else {
      $indel_rate_close_enough = 1;
    }

#     if ($sim_aln_length < $aln_length && ((1.2 * $sim_aln_length) < $aln_length)) {
#       $residues_length = $residues_length * 1.2;
#     } elsif ($sim_aln_length > $aln_length && ((0.8 * $sim_aln_length) > $aln_length)) {
#       $residues_length = $residues_length * 0.8;
#     } else {
#       $aln_length_close_enough = 1;
#     }

    if ($sim_aln_percent_identity < $aln_percent_identity && ((1.2 * $sim_aln_percent_identity) < $aln_percent_identity)) {
      $total_branch_length = 0;
      foreach my $subnode ($self->{tree}->get_all_subnodes) {
        my $dist = $subnode->distance_to_parent;
        $dist = 0.000000001 if (0 == $dist); $dist = $dist*0.8;
        $subnode->distance_to_parent($dist);
        $total_branch_length += $dist;
        $newick = $self->{tree}->newick_format('member_id_taxon_id');
        $newick =~ s/(:0\.?0+)([;,()])/:0.0005$2/g; $newick =~ s/:\d\.?\d+;/;/g;
      }
    } elsif ($sim_aln_percent_identity > $aln_percent_identity && ((0.8 * $sim_aln_percent_identity) > $aln_percent_identity)) {
      $total_branch_length = 0;
      foreach my $subnode ($self->{tree}->get_all_subnodes) {
        my $dist = $subnode->distance_to_parent;
        $dist = 0.000000001 if (0 == $dist); $dist = $dist*1.2;
        $subnode->distance_to_parent($dist);
        $total_branch_length += $dist;
        $newick = $self->{tree}->newick_format('member_id_taxon_id');
        $newick =~ s/(:0\.?0+)([;,()])/:0.0005$2/g; $newick =~ s/:\d\.?\d+;/;/g;
      ...
      $subst_rate_close_enough = 1;
    }
    print STDERR "[indel_rate_close_enough $indel_rate_close_enough subst_rate_close_enough $subst_rate_close_enough aln_length_close_enough $aln_length_close_enough]\n" if ($self->debug);
    $iterations++;
    last if ($iterations > 10);
  }
  $tree->store_tag('sim_aln_num_residues',$sim_aln_num_residues);
  $tree->store_tag('sim_aln_length',$sim_aln_length);
  $tree->store_tag('sim_aln_percent_identity',$sim_aln_percent_identity);
  $tree->store_tag('sim_iterations',$iterations);
...
sub run_magicmirror {
  ...
  if (defined($self->{store_this})) {
    my $subdir = $self->{store_this};
    my $ret2 = `cp $transcriptsfile $subdir/`;
  }
